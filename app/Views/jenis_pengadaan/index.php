...
<div class="card card-primary card-outline">
  <div class="card-header"><h3 class="card-title"><?= $title ?></h3>
    <div class="card-tools"><a href="<?= base_url('jenis-pengadaan/create') ?>" class="btn btn-sm btn-primary"><i class="fas fa-plus"></i> Tambah</a></div>
  </div>
    ...
    <table class="table table-bordered table-striped table-hover dataTable" id="tabelData">
      ...
      <tbody>
        ...
        <tr><td><?= $no++ ?></td><td><?= esc($row['nama']) ?></td><td><?= esc($row['deskripsi'] ?? '-') ?></td>
          ...
          <td>
            <a href="<?= base_url("jenis-pengadaan/{$row['id']}/edit") ?>" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
            <button class="btn btn-sm btn-danger btn-hapus" data-url="<?= base_url("jenis-pengadaan/{$row['id']}/delete") ?>" data-nama="<?= esc($row['nama']) ?>"><i class="fas fa-trash"></i></button>
          </td>
        </tr>
        <?php endforeach; ?>
      </tbody>
